...
|[[valgrind](#5.3.1.valgrind)] | [[openmp](#5.3.2.openmp)] | [[simd](#5.3.3.simd)]
...
[c++ map使用事项](https://www.cnblogs.com/nzbbody/p/3409298.html)
...
头文件地址：googletest/include, googlemock/include
```
+ 使用
...
+ OpenMP如果不是累加操作（reduction）, 而是简单的并行操作并且操作相同的变量，性能会很差，因为涉及资源争抢；

<h3 id="5.3.3.simd">5.3.3. simd</h3>

SIMD全称为单指令多数据流，一种指令加速方式。

SSE通常包括3个阶段：数据加载，计算，结果保存。

数据加载(load/set系列函数)，是否按照16字节对齐，可分别使用：

```c++
_mm_load_ps(const float *)  // 必须对齐字节, 如_mm_load_ps(v), 使用_mm_load_ps(v+1)报错；
_mm_loadu_ps(const float*) // 无需对齐，_mm_load_ps(v+1)正确。适用于FM/FFM 隐向量内积计算
```
